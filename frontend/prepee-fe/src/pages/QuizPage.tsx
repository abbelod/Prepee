  ...
  const handleSubmit = () => {
    ...
    const postSubmission = async () => {
      ...
      try {
        ...
        const response = await fetch(`https://prepee.onrender.com/matchmaking/${matchData.matchId}/submit-answer/`, {
          method: 'POST',
          headers: {
        ...
        if (data.status === 'waiting') {
          ...
          pollTimerRef.current = window.setInterval(async () => {
            try {
              const pollRes = await fetch(`https://prepee.onrender.com/matchmaking/${matchData.matchId}/submit-answer/`, {
                method: 'POST',
                headers: {
                  'Content-Type': 'application/json',
                  Authorization: `Bearer ${token}`,
